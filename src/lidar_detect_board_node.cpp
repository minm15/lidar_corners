...
#include <cmath> 
#include "lidar_detect_board_node.hpp"
#include "GroundRemoval.h"

using std::placeholders::_1;


LidarPlaneExtractNode::LidarPlaneExtractNode()
: Node("lidar_detect_board_node")
{
    // === Get Parameters ===
    ...
    this->declare_parameter<double>("dist", 30.0);
    ...
    this->get_parameter("dist", dist_limit_);
...
void LidarPlaneExtractNode::cloudCallback(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
{
    // A. ROS -> PCL
    ...
    for (auto &pt : raw_cloud->points)
    {
        ...
        float dist_squared = pt.x * pt.x + pt.y * pt.y + pt.z * pt.z;
        if (angle_deg >= -40.0f && angle_deg <= 40.0f && dist_squared <= dist_limit_ * dist_limit_)
        {
    ...
    // Publish the result
    planes_aggregate->width = planes_aggregate->points.size();
    planes_aggregate->height = 1;
    planes_aggregate->is_dense = false;
